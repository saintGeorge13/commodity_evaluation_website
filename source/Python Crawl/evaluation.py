    ...
    if "error_code" not in data or data["error_code"] == 0:
        ...
        for item in data["items"]:
            ...
            if item["prop"].find("一般") != -1:
                ...
                item["adj"] += "一般"
            comments.append(item["prop"] + item["adj"])
        ...
        for j in range(len(comments)):
            ...
            if(useful):
                # 换成数据库
                statement = """INSERT INTO evaluation_new(adj, prop, sentiment, comment_id) VALUES ('{0}', '{1}', '{2}', '{3}')""".format(
                data["items"][j]["adj"], data["items"][j]["prop"], data["items"][j]["sentiment"], id)
                cursor = conn.cursor()
